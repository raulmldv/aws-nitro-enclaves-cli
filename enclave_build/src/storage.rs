...
pub const STORAGE_ROOT_FOLDER: &str = "XDG_DATA_HOME";
/// Path to the blobs folder
pub const STORAGE_BLOBS_FOLDER: &str = "blobs/sha256/";
/// Name of the storage index file which stores the (image URI <-> image hash) mappings.
pub const STORAGE_INDEX_FILE_NAME: &str = "index.json";
/// The name of the OCI layout file from the storage.
pub const STORAGE_OCI_LAYOUT_FILE: &str = "oci-layout";
...
pub struct OciStorage {
    ...
    /// The config of the OCI image used to build the EIF
...
impl OciStorage {
    pub fn new(root_path: &Path) -> Result<Self> {
        ...
        fs::create_dir_all(&root_path).map_err(EnclaveBuildError::OciStorageInitError)?;
    ...
    pub fn store_image_data(&mut self, image_name: &str, image_data: &ImageData) -> Result<()> {
        ...
        fs::create_dir_all(&blobs_path).map_err(EnclaveBuildError::OciStorageStoreError)?;
    ...
    fn store_config(blobs_path: &Path, image_data: &ImageData) -> Result<String> {
        let config_content = String::from_utf8(image_data.config.data.clone()).map_err(|_| {
            EnclaveBuildError::OciStorageStoreError(Error::new(
    ...
    fn store_index(&self, image_name: &str, manifest: &OciImageManifest) -> Result<()> {
        ...
        let index_file = File::options()
            ...
            .map_err(EnclaveBuildError::OciStorageStoreError)?;
    ...
    fn check_stored_image(&self, image_name: &str) -> Result<ImageConfiguration> {
        ...
        if config_digest != format!("{:x}", sha2::Sha256::digest(config_string.as_bytes())) {
            return Err(EnclaveBuildError::OciStorageMalformedError(
    ...
    fn validate_layers(&self, manifest_obj: &OciImageManifest) -> Result<()> {
        ...
        fs::read_dir(layers_path)
            .map_err(|err| {
                EnclaveBuildError::OciStorageNotFoundError(format!(
    ...
    fn check_layer(layer: &OciDescriptor, blobs: &HashMap<String, File>) -> Result<()> {
        ...
        let layer_ref = layer.digest.strip_prefix("sha256:").ok_or_else(|| {
            EnclaveBuildError::OciStorageMalformedError("Layer digest incorrect.".to_string())
        })?;
        let mut layer_file = blobs.get(layer_ref).ok_or_else(|| {
            EnclaveBuildError::OciStorageNotFoundError("Layer missing from storage.".to_string())
        })?;
        let mut layer_string = String::new();
        layer_file.read_to_string(&mut layer_string).map_err(|_| {
            EnclaveBuildError::OciStorageNotFoundError("Failed to read layer".to_string())
        })?;

        let calc_digest = format!("sha256:{:x}", sha2::Sha256::digest(layer_string.as_bytes()));
    ...
    fn fetch_index(root_path: &Path) -> Result<OciImageIndex> {
        match File::open(root_path.join(STORAGE_INDEX_FILE_NAME)) {
            ...
            Err(err) => Err(EnclaveBuildError::OciStorageNotFoundError(format!(
                "Index file missing: {:?}",
                err
            ))),
        }
...
pub mod tests {
    ...
    fn setup_temp_storage() -> (PathBuf, OciStorage) {
        ...
        let mut storage = OciStorage::new(&root_dir).expect("failed to create the OciStorage");
    ...
    fn test_image_is_valid() {
        ...
        res.expect("Validation failed");
    }

    #[test]
    fn test_image_is_not_valid() {
        let (root_path, storage) = setup_temp_storage();
    ...
    #[test]
    fn test_fetch_index() {
        let (root_path, _storage) = setup_temp_storage();
