...
pub fn new_enclave_name(run_args: RunEnclavesArgs) -> NitroCliResult<String> {
    ...
    let mut names = get_all_enclave_names().map_err(|e| {
...
pub fn get_id_by_name(name: String) -> NitroCliResult<String> {
    ...
    let mut replies: Vec<UnixStream> = vec![];
    replies.extend(comms);
    let objects: Vec<String> = enclave_proc_handle_outputs::<String>(&mut replies)
        .iter()
        .map(|v| v.0.clone())
        .collect();
    ...
    if objects.is_empty() {
        return Err(new_nitro_cli_failure!(
            "No enclave matched the given name.".to_string(),
            NitroCliErrorEnum::EnclaveNamingError
        ));
    } else if objects.len() > 1 {
        return Err(new_nitro_cli_failure!(
            "Conflicting enclave names have been found.".to_string(),
            NitroCliErrorEnum::EnclaveNamingError
        ));
    }
